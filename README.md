...
游戏式神经进化：生物体（LIF 网络，纯 numpy、一生无学习、权重出生即随机）住在培养皿里吃数字、产出数字。**回合制**规则（一回合 = 喂 1 个数字 → 全部存活个体各自产出 → 产错概率死亡 → 繁殖 → 结算）：

- **一回合喂 1 个随机数字**：所有存活个体同时看到同一个数字并各自产出。
- **产错按已存活回合数概率死亡**（首回合 100% 即死，存活 1 回合 → 50%，每多存活一回合死亡概率 ×1.5、封顶 100%）；未死但 **age > 存活回合数上限 → 自然死亡**（计停止指标）。
- **选型交配**：存活者按**年龄相似度**加权配对（强度 s 可调，s=0 退化为随机）——活得久的个体更倾向与同样活得久的繁殖，集中存活基因。
- **存活奖励 alpha**：`alpha = 1 / 本回合存活率`（种群级、全局）。存活率越低（残酷回合），存活者繁殖倍数越高；`每对产仔数 = 存活回合数 × N × alpha × 密度`。
- **存活时长加权交叉**：存活更久的亲代提供子代架构；同位置形状兼容层逐权重 50/50 + 均匀小扰动 + 千分之一大突变。
- **五类结构突变**（出生时按概率施加，边界钳制）：静默神经元诞生（近零、行为不变）/ 层复制恒等中继 / 层合并 / 神经元剪枝 / 随机整层插入 → 可演化出 **1–4 隐藏层**的变深度架构（初始单层 100）。
- **密度依赖 + 承载力封顶**：种群硬上限 = 承载力；繁殖 = 覆盖当回合死亡 + 密度加权净增长（logistic 近似），种群向承载力缓慢增长。
- **全灭重播**：某回合全员死亡则按初始种群数重新播种。
- **停止条件**：累计自然死亡 / 累计总死亡 ≥ 95% 停止推演（随机权重下基本不可达）。
...
| 繁殖倍数 N | 10–100 | 50 | 每对每回合繁殖量 = 存活回合数 × N（再乘 alpha × 密度） |
| 选型强度 s | 0–100% | 50% | 年龄相似度配对的强度（0=随机，100%=纯按年龄） |
| 承载力 | 100–10000 | 10000 | 种群硬上限 |
| 初始种群 | 60–1000 | 1000 | 初始 / 全灭重播种群数 |

运行：
    python eco_server.py --port 8765
浏览器打开 http://127.0.0.1:8765 即可观看培养皿动画（只显示存活个体，死亡当回合即移除）、
手动喂食、个体解剖（含架构图）、种群统计（历史曲线 + 存活回合数直方图）。

**诚实基线（随机权重、无学习）**：正确率贴随机线（best ≈ 0.1–0.2）；多层个体能出现并受生态选择存活
（冒烟 30 回合可达种群 ~35%）；natural_rate 远低于 0.95 停止目标（基本不可达）；单回合耗时随种群线性增长（pop 1000 时约 0.3s）。
