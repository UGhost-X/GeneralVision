...
游戏式神经进化：生物体（LIF 网络，纯 numpy、一生无学习、权重出生即随机）住在培养皿里吃数字、产出数字。**回合制**规则（一回合 = 喂 1 个数字 → 全部存活个体各自产出 → 按正确率淘汰 → 繁殖 → 结算）：

- **一回合喂 1 个随机数字**：所有存活个体同时看到同一个数字并各自产出。
- **按正确率排淘汰最差 30%**（非自然死亡，不计停止指标）；存活顶部 70% 中 **age > 存活回合数上限 → 自然死亡**（计停止指标）。
...
- **密度依赖 + 承载力封顶**：种群硬上限 = 承载力；v3 不强制回填满容量，种群稳定在初始规模附近（繁殖只覆盖当回合死亡）。
